...
char* getOpType(char operand[]) {
  ...
  char err[] = "ERROR OPERAND NOT FOUND";
  error(err);
  return nullptr;
  /* In theory we can do this because we add every literal to the symbol table
   * If this is not working, then the symbol table is not handling non char values[] very well
   *
   */ 
}
...
{  
  ...
  std::string type2 = getOpType(operand2);
